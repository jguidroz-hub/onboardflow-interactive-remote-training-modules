...
export default function EmployeeProgressPage() {
  const [items, setItems] = useState<EmployeeProgressItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/employee-progress')
      .then(r => r.json())
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/employee-progress/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Employee Training Progress</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Track and analyze employee learning</p>
